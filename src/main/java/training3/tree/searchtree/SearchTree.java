package training3.tree.searchtree;

import java.util.*;

public class SearchTree {

    public static boolean solution(Tree<Integer> tree, int target) {
        return isExist(tree, target);
    }

    public static boolean isExist(Tree<Integer> tree, int target) {
        if (tree == null) return false;
        if(tree.value == target) return true;
        if(tree.value > target) {
            return isExist(tree.left, target);
        } else {
            return isExist(tree.right, target);
        }
    }

package training2.graph.circlegraphcheck;
...
public class CircleGraphCheck {
    public static boolean solution(int[][] connections) {
        for (int i = 0; i < connections.length; i++) {
            if (dft(connections,i,new HashSet<>())) return true;
        }
        return false;
    }
    ...
    public static boolean dft(int[][] connections, int start, Set<Integer> visited) {
        ...
        for (int i = 0; i < connections[start].length; i++) {
            if (dft(connections,connections[start][i],newVisited)) {
                return true;
            }
        }

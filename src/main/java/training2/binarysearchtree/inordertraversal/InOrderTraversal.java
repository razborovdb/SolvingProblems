package training2.binarysearchtree.inordertraversal;
...
public class InOrderTraversal {
    ...
    public static int[] solution(Tree<Integer> tree) {
        ...
        list = recursion(tree,list);
        return list.stream().mapToInt(Integer::intValue).toArray();
    }

    public static List<Integer> recursion(Tree<Integer> tree, List<Integer> list) {
        if (tree == null) return list;
        list.add(tree.value);
        if (tree.left != null) {
            recursion(tree.left, list);
        }
        if (tree.right != null) {
            recursion(tree.right, list);
        }
        return list;
    }
